...
for kPP1_ = [.25, .5, 1, 2, 4, 8, 16]
    for kCaMKII_ = [.05, .1, .25, .5, 1, 2, 4]
        ...
        for constantCa = caSweep
            ...
            %Execute Model
            [tVec,rxnOut] = Zhabotinsky2000_CaM4(start, stop, stepSize, altstepSize, subunits, ca2Func, ca2Const, alpha, rateConst, false);
...
FileName = strcat('Results\OutputDutyCurves_',datestr(datetime('now'),'mm-dd_HH-MM-SS'),'.txt');
