%% INPUTS: bool ruled, double cPP1
function ModelSweeper_kPP1_kCaMKII_cPP1in(ruled, cPP1)
...
CaM0 = 25; %[CaM] @t=0, 1uM 
ep0 = str2num(cPP1); %[PP1] @t=0, .01-1.2uM
...
for kPP1_ = [.25, .5, 1, 2, 4, 8, 16]
    for kCaMKII_ = [.05, .1, .25, .5, 1, 2, 4]
        ...
        for constantCa = caSweep
            ...
            %Execute Model
            [tVec,rxnOut] = Zhabotinsky2000_CaM4(start, stop, stepSize, altstepSize, subunits, ca2Func, ca2Const, alpha, rateConst, str2num(ruled));
...
FileName = strcat('Results\OutputDutyCurves_PP1_',num2str(cPP1*100),'_ruled',num2str(ruled),'.txt');
